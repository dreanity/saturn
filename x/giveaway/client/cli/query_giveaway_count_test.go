...
func networkWithGiveawayCountObjects(t *testing.T) (*network.Network, types.GiveawayCount) {
	t.Helper()
...
func TestShowGiveawayCount(t *testing.T) {
	net, obj := networkWithGiveawayCountObjects(t)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdShowGiveawayCount(), args)
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetGiveawayCountResponse
				require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
				require.NotNil(t, resp.GiveawayCount)
				require.Equal(t,
					nullify.Fill(&tc.obj),
					nullify.Fill(&resp.GiveawayCount),
				)
			}
		})
	}
