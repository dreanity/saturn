package gentime_test

import (
	"testing"

	keepertest "github.com/dreanity/saturn/testutil/keeper"
	"github.com/dreanity/saturn/testutil/nullify"
	"github.com/dreanity/saturn/x/gentime"
	"github.com/dreanity/saturn/x/gentime/types"
	"github.com/stretchr/testify/require"
)

func TestGenesis(t *testing.T) {
	genesisState := types.GenesisState{
		Params: types.DefaultParams(),

		Time: types.Time{
			Value: 56,
		},
		// this line is used by starport scaffolding # genesis/test/state
	}

	k, ctx := keepertest.GentimeKeeper(t)
	gentime.InitGenesis(ctx, *k, genesisState)
	got := gentime.ExportGenesis(ctx, *k)
	require.NotNil(t, got)

	nullify.Fill(&genesisState)
	nullify.Fill(got)

	require.Equal(t, genesisState.Time, got.Time)
	// this line is used by starport scaffolding # genesis/test/assert
}
